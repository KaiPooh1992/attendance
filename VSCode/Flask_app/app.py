...
tempate_folder = os.path.join(os.path.dirname(__file__),"templates/")
...
def validate_sign_up():
    ...
    encrypt_pass = hashlib.md5(passwd.encode()).hexdigest() #make password or passwd hidden
...
def gallery():
    ...
    return render_template("gallery.html", names=record)
...
def gallery_add_post():
    ...
    if link !="" and category !="":
        ...
        VALUES (?,?,?,?)
        '''
        val = (link,filename.filename,category,detail)
        cur.execute(sql_insert, val)
        conn.commit()
        conn.close
...
def gallery_edit(gallery_id):
    ...
    record = cur.fetchone()
    conn.close()
    return render_template('gallery-edit.html', name=record)
...
def gallery_edit_post(gallery_id):
    ...
    if link !="" and category !="":
        ...
        if filename.filename !="":
            ...
            sql_update = '''
            UPDATE gallery
            SET link=?, category=?, detail=?, image=?
            WHERE gallery_id=?
            '''
...
def product():
    ...
    return render_template("product.html", names=record)
...
def product_add_post():
    ...
    if title !="" and detail !="":
        ...
        VALUES (?,?,?,?)
        '''
        val = (title, detail, price, filename.filename)
        cur.execute(sql_insert, val)
        conn.commit()
        conn.close
...
def product_edit(product_id):
    ...
    record = cur.fetchone()
    conn.close()
    return render_template('product-edit.html', name=record)
